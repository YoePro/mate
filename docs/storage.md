...
0.15 stores a lightweight `Network.domain` property. Existing networks without a stored domain are treated as `social`.
...
`social` networks use global person identity behavior and duplicate detection. `flowchart` is a non-social diagram domain backed by network-scoped `DiagramNode` records.

Diagram nodes are not global identities. They belong to one network and store:
...
- Flowchart/diagram nodes are local diagram artefacts, so delete physically removes the diagram node, its network-scoped relationships, and its saved position instead of archiving it.
